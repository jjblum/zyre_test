...
{
    // std::this_thread::sleep_for(std::chrono::milliseconds(50)); // pretend to work
    std::cout << "Thread \"" << m_name << "\": listening for zmsg's..." << std::endl;
    zyre::zmsg msg(m_pNode->recv()); // blocks until a zmsg is received - can even prevent the thread from shutting down!
    if (!msg.is_zmsg()) return; 
